...
public class Vehicle_vcno extends JFrame implements ActionListener,WindowListener,KeyListener
{
...
//	}
	...
	public void GUI() {
		...
		lbl_parkingID.setBounds(57, 225, 121, 29);
		...
		lbl_VehicleNO.setBounds(57, 61, 121, 29);
		...
		text_ParkingID.setBounds(188, 227, 173, 29);
		...
		text_VehicleNO.setBounds(188, 63, 173, 29);
	 ...
	 public void fetchDetails()
	   {
		   ...
		   vcno = text_VehicleNO.getText().strip();
		...
		try 
		  {
			  ...
			  String str_read = "select * from vehicledetails where vehiclenumber=?";
			  psread = con.prepareStatement(str_read);
			  psread.setString(1, vcno);
			  rsread = psread.executeQuery();
			  int flag = 0;
			 while(rsread.next())
			  {
				  
				  String vehicle =rsread.getString("vehiclenumber");
				  if(vehicle.equalsIgnoreCase(vcno))
				  {
					flag = 1; 
					break;
				  }
			 ...
			 if(flag == 1)
			 {
			  ...
			  text_Status.setText(rsread.getString("status"));
				 ...
				 JOptionPane.showMessageDialog(this,"No vehicle found", "Error", JOptionPane.ERROR_MESSAGE);
